...
const handleConvertAgain = async () => {
  if (!window.confirm("This potential customer is already converted. Do you want to convert them again? This will create a NEW customer record and remove this potential customer from the list.")) {
    return;
  }

  try {
    // Step 1: Convert again (create new customer)
    const res = await axiosInstance.post(`/potential-customers/${id}/convert`);
    toast.success("Converted again successfully!");
    
    // Step 2: Delete the potential customer record
    await axiosInstance.delete(`/potential-customers/${id}`);
    toast.success("Potential customer record removed from list");
    
    // Step 3: Navigate to the new regular customer edit page
    navigate(`/customers/edit/${res.data.customer._id}`);
  } catch (err) {
    console.error("Convert again error:", err);
    toast.error(err.response?.data?.error || "Failed to convert again");
  }
};
...
<div className="flex gap-2 flex-wrap">
  ...
  {/* ✅ NEW: Convert Again Button - Show only if ALREADY converted */}
  {customer && customer.convertedToCustomerId && (
    <button
      type="button"
      onClick={handleConvertAgain}
      className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg transition-colors duration-200 shadow-sm"
      title="Convert this potential customer to a regular customer again (this will create a new customer record)"
    >
      🔄 Convert to Customer
    </button>
  )}
