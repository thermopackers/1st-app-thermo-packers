...
export default function FactoryActCertificates() {
  ...
  const fetchCertificates = async () => {
    try {
      ...
      const res = await axiosInstance.get(`/factory-act-certificate?${params.toString()}`);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const res = await axiosInstance.post("/factory-act-certificate/upload", uploadData, {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.certificateName) {
      Swal.fire("Error", "Please enter certificate name", "error");
      return;
    }
    ...
    try {
      ...
      let res;
      if (editingId) {
        res = await axiosInstance.put(`/factory-act-certificate/${editingId}`, submitData);
      } else {
        res = await axiosInstance.post("/factory-act-certificate", submitData);
      }
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axiosInstance.delete(`/factory-act-certificate/${id}`);
      ...
      <div className="max-w-7xl mx-auto p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Factory License, Boiler Certificate, etc."
                    required
                  />
                </div>
            ...
            <table className="min-w-full border-collapse">
              <thead className="bg-slate-200">
                <tr>
                  ...
                  <th className="border p-3 text-left text-xs">Certificate Name</th>
              ...
              <tbody>
                {certificates.map((cert, index) => (
                  <tr key={cert._id} className="hover:bg-slate-50">
                    ...
                    <td className="border p-2 text-xs font-medium">{cert.certificateName}</td>
        ...
        {!loading && certificates.length === 0 && (
          <div className="text-center py-8 text-gray-500 bg-white rounded-lg shadow">
            ...
            <p className="mt-2">Click "Add New Certificate" to create your first entry.</p>
          </div>
        )}
