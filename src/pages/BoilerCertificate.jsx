...
export default function BoilerCertificate() {
  ...
  const fetchCertificates = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get(`/boiler-certificate?page=${page}&limit=10`);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const res = await axiosInstance.post("/boiler-certificate/upload", uploadData, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let res;
      if (editingId) {
        res = await axiosInstance.put(`/boiler-certificate/${editingId}`, submitData);
      } else {
        res = await axiosInstance.post("/boiler-certificate", submitData);
      }
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axiosInstance.delete(`/boiler-certificate/${id}`);
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">
              Boiler Yearly Certificate Management
            </h1>
