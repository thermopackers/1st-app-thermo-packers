...
export default function ESICManagement() {
  ...
  const [editingDoc, setEditingDoc] = useState(null); // For edit mode
  ...
  const [existingChallan, setExistingChallan] = useState(null); // Store existing file info
  ...
  const [existingReceipt, setExistingReceipt] = useState(null); // Store existing file info
  ...
  const fetchDocuments = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get(`/esic?year=${selectedYear}`);
      if (res.data.success) {
        setDocuments(res.data.documents);
      }
    } catch (err) {
      console.error("Error fetching ESIC documents:", err);
      Swal.fire("Error", "Failed to load ESIC documents", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleChallanUpload = async (e) => {
    ...
    try {
      const res = await axiosInstance.post("/esic/upload-challan", formData, {
  ...
  const handleReceiptUpload = async (e) => {
    ...
    try {
      const res = await axiosInstance.post("/esic/upload-receipt", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axiosInstance.post("/esic", submitData);
      
      if (res.data.success) {
        Swal.fire({
          title: "Success!",
          text: editingDoc ? "ESIC documents updated successfully" : "ESIC documents saved successfully",
          icon: "success",
          timer: 2000,
          showConfirmButton: false
        });
      ...
      Swal.fire("Error", err.response?.data?.message || "Failed to save ESIC documents", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteChallan = async (docId) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axiosInstance.delete(`/esic/challan/${docId}`);
  ...
  const handleDeleteReceipt = async (docId) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axiosInstance.delete(`/esic/receipt/${docId}`);
        ...
        <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">
              ESIC Document Management
            </h1>
            <p className="text-gray-600 text-xs mt-1">
              Manage Monthly Challan and Payment Receipt documents
            </p>
          </div>
          ...
          {showForm && (
            ...
            >
              <h2 className="text-xl font-bold mb-4">
                {editingDoc ? `Edit Documents for ${months.find(m => m.value === selectedMonth)?.name}` : "Add ESIC Documents"}
              </h2>
