...
export default function DriverDispatchDashboard() {
  ...
  // Fetch all vehicles for driver to select from
  const fetchVehicles = async () => {
    ...
    try {
      ...
      // Auto-select the driver's assigned vehicle if available
      const driverVehicle = res.data.find(v => v.driverEmail === user.email);
      if (driverVehicle) {
        setSelectedVehicle(driverVehicle.vehicleNumber);
      } else if (res.data.length > 0) {
        setSelectedVehicle(res.data[0].vehicleNumber);
      }
    } catch (err) {
  ...
  const fetchPlans = async (currentPage = 1) => {
    ...
    try {
      const res = await axiosInstance.get(
        `/dispatch-plans/my-plans?page=${currentPage}&limit=${limit}&vehicle=${selectedVehicle}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      Swal.fire("Error", "Failed to load dispatch plans", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Refetch plans when selected vehicle changes
  useEffect(() => {
    if (selectedVehicle) {
      fetchPlans(1);
    } else {
  ...
  const openDieselEntryModal = async (plan, existingEntry = null) => {
    ...
    if (formValues) {
      try {
        ...
        fetchDieselEntries(); // Refresh diesel entries
      } catch (err) {
              ...
              >
                ...
                {vehicles.map((vehicle) => (
                  <option key={vehicle._id} value={vehicle.vehicleNumber}>
                    ...
                    {vehicle.driverEmail === user?.email && " (Your Vehicle)"}
            ...
            {selectedVehicle && (
              <div className="bg-blue-50 px-4 py-2 rounded-lg border border-blue-200">
                <p className="text-sm text-blue-700 font-medium">
                  👨‍✈️ Currently viewing plans for: <strong>{selectedVehicle}</strong>
                </p>
              </div>
            )}
          </div>
        ...
        {vehiclesLoading ? (
          ...
          <div className="text-center py-12 bg-yellow-50 rounded-xl border border-yellow-200">
            ...
            <p className="text-yellow-600">
              Please select a vehicle from the dropdown above to view dispatch plans.
            </p>
          </div>
        ) : loading ? (
          <div className="flex justify-center items-center min-h-[200px] transition-opacity duration-300">
            ...
            <span className="ml-3 text-blue-600">
              Loading dispatch plans for {selectedVehicle}...
            </span>
          </div>
        ) : (
  ...
  {plan.expenses && (
    <p className="sm:col-span-1">
      <strong>💰 Expenses:</strong> ₹{plan.expenses}
        ...
        {!loading && plans.length === 0 && selectedVehicle && (
          <p className="text-center text-gray-500 mt-8">
            No dispatch plans assigned for {selectedVehicle}.
          </p>
        )}
      </div>
