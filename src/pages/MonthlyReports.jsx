...
export default function MonthlyReports() {
  ...
  // UPDATED: renderPresentDetails with On Tour support
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-wrap gap-4 mb-4 text-sm">
          ...
          <div className="flex items-center">
            <div className="w-3 h-3 bg-amber-300 rounded mr-2"></div>
            <span>🌍 On Tour</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-7 gap-1 mb-6">
          ...
          {allDates.map(dateStr => {
            ...
            const isOnLeave = leaveDateMap[dateStr];
            const isOnTour = presentDateMap[dateStr]?.onTour || false;
            ...
            if (isOnLeave) {
              ...
              if (isSunday) {
                ...
                // Check for On Tour
                const onTourText = isOnTour ? " 🌍 (On Tour)" : "";
                ...
                titleText = isHalfDay ? `Half Day${reasons.length ? ' (' + reasons.join(', ') + ')' : ''}${onTourText}` : `Full Day${onTourText}`;
              }
            ...
            return (
              <div 
                key={dateStr} 
                className={`p-2 rounded text-center text-sm ${bgColor} ${textColor} relative cursor-help ${isOnTour ? 'border-2 border-amber-400' : ''}`}
                title={titleText}
              >
                {dayOfMonth}
                {isOnLeave && (
        ...
        {/* Attendance Details Table with On Tour Column */}
          ...
          <>
            <div className="flex justify-between items-center mb-3">
              ...
              <div className="flex gap-2 text-sm flex-wrap">
                ...
                {onTourCount > 0 && (
                  <span className="px-3 py-1 bg-amber-200 text-amber-800 rounded-full font-medium">
                    🌍 On Tour: {onTourCount}
                  </span>
                )}
              </div>
              ...
              <table className="min-w-full border border-gray-200 rounded-lg overflow-hidden">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left text-sm font-medium text-gray-800">Mode</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {allAttendanceRecords.map((record, index) => {
                    ...
                    const isOnTour = record.onTour || false;
                    ...
                    if (record.type === 'leave') {
                      ...
                      if (isSunday) {
                        ...
                        // Add On Tour text to attendance type
                        const onTourText = isOnTour ? " 🌍" : "";
                        ...
                        attendanceType = isHalfDay ? `Half Day${reasons.length ? ' (' + reasons.join(', ') + ')' : ''}${onTourText}` : `Full Day${onTourText}`;
                ...
                <tfoot className="bg-gray-50 border-t-2 border-gray-300">
                  <tr>
                    <td colSpan="6" className="px-3 py-3 text-sm font-semibold text-gray-800 text-right">
                      Total Days: {allAttendanceRecords.length} | 
                    </td>
                    <td className="px-3 py-3 text-sm">
                      <span className="font-semibold text-gray-800">
                        FD: {fdCount} | HD: {hdCount} | Leave: {totalLeaveCount} | 🌍 On Tour: {onTourCount}
                      </span>
                    </td>
                  </tr>
                </tfoot>
  ...
  // Add this helper function for PDF download with On Tour support
const downloadAttendanceTable = async (attendanceRecords, month, userId, fdCount, hdCount, leaveCount) => {
  ...
  try {
    ...
    // Title
    doc.setFontSize(16);
    doc.text(`Attendance Report - ${userName}`, 14, 20);
    doc.setFontSize(11);
    ...
    const tableData = allDatesInMonth.map(dateStr => {
      ...
      if (!attendanceType) {
        if (isSunday) {
          ...
          // Add On Tour indicator
          const onTourText = isOnTour ? " (On Tour)" : "";
          
          if (isHalfDay) {
            if (isIncomplete) {
              attendanceType = `Half Day${onTourText}`;
            ...
            attendanceType = `Full Day${onTourText}`;
          }
        } else {
      ...
      return [
        ...
        hours,
        attendanceType,
        isOnTour ? "On Tour" : "Office"
      ];
    });
    
    // Generate table with proper column widths
    autoTable(doc, {
      head: [['Date', 'Day', 'Check-In', 'Check-Out', 'Hours', 'Status', 'Mode']],
      body: tableData,
      startY: 42,
      theme: 'striped',
      headStyles: { 
      ...
      columnStyles: {
        ...
        1: { cellWidth: 18, halign: 'center' },   // Day
        2: { cellWidth: 25, halign: 'center' },   // Check-In
        3: { cellWidth: 25, halign: 'center' },   // Check-Out
        4: { cellWidth: 20, halign: 'center' },   // Hours
        5: { cellWidth: 35, halign: 'left' },     // Status (left align for longer text)
        6: { cellWidth: 20, halign: 'center' }    // Mode
      },
      margin: { top: 42, bottom: 50, left: 12, right: 12 },
      pageBreak: 'auto',
      rowPageBreak: 'avoid',
      didParseCell: function(data) {
        // Highlight On Tour rows
        if (data.section === 'body' && data.column.index === 6) {
          if (data.cell.raw === 'On Tour') {
            data.cell.styles.fillColor = [255, 235, 59]; // Amber background
    ...
    const onTourDays = attendanceRecords.filter(r => r.onTour && r.type === 'present').length;
    ...
    // Line 2
    doc.text(`Sundays: ${sundays.length} total (${sundays.length} off)`, 14, currentY);
    currentY += 7;
    ...
    // Line 4
    doc.text(`Present: ${presentDates.length} | Absent: ${absentDays.length}`, 14, currentY);
    currentY += 7;
    ...
    // Line 6 - FIXED: Removed emoji
    if (onTourDays > 0) {
      doc.text(`On Tour Days: ${onTourDays}`, 14, currentY);
      currentY += 7;
    }
    ...
    if (leaveCount > 0) {
      ...
      doc.text(`Leaves: ${leaveCount} total (Net: ${netLeaves})`, 14, currentY);
    } else {
      doc.text(`Leaves: 0`, 14, currentY);
    }
