...
export default function MainElectricPanelPageUnit2() {
...
const fetchWeeklyAverage = async () => {
  try {
    ...
    const res = await axiosInstance.get("/main-electric-panel-unit2/weekly-average"); // Change endpoint for Unit2/Unit3
  ...
  const fetchData = async (p = page) => {
    try {
      const res = await axiosInstance.get(
        `/main-electric-panel-unit2?page=${p}&limit=${limit}`
      );
...
if (updated[i].netKwh !== undefined && updated[i].netKvah !== undefined && updated[i].netKvah !== 0) {
  ...
  const handleSave = async (row) => {
    try {
      setSaving(true);
      await axiosInstance.post("/main-electric-panel-unit2", row);
...
const handleDeleteAttachment = async (rowIndex, attachmentIndex, attachment) => {
  try {
    ...
    if (attachment.public_id) {
      await axiosInstance.post("/main-electric-panel-unit2/delete-cloudinary-file", { 
  ...
  try {
    ...
    if (row.attachments && row.attachments.length) {
      const deletePromises = row.attachments
        ...
        .map(attachment => 
          axiosInstance.post("/main-electric-panel-unit2/delete-cloudinary-file", { 
            public_id: attachment.public_id 
          })
        );
    ...
    await axiosInstance.delete(`/main-electric-panel-unit2/${row._id}`);
    ...
    <div className="max-w-6xl mx-auto px-4">
      ...
      <div className="p-2 sm:p-4 relative">
        <h1 className="text-xl font-bold mb-4 text-center sm:text-left">
          THERMO PACKERS - Unit 2 - Daily Electricity PF (Power Factor Report)
        </h1>
<h2 className="text-xl bg-red-200 p-1 rounded font-bold mb-4 text-center sm:text-left">
PSPCL Account No: 3002901879
        </h2>
        <div className="flex flex-col sm:flex-row justify-between items-center mb-2 gap-2">
          <h2 className="text-lg font-semibold">Main Electric Panel - Unit 2</h2>
          <button
            onClick={addNewRow}
...
className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-4 h-4 text-xs"
      >
        ...
        <YouTubeGuide 
      videoId="TcpRSScCcXU" 
      title="Learn How to take Power Factor Readings - Unit 2"
    />
    </>
  );
}