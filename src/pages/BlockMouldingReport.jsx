    ...
    const fetchData = async () => {
      ...
      try {
        const res = await axiosInstance.get(
          "/production-reports-block/block-moulding",
          {
            params: {
              page: currentPage,
              limit: ROWS_PER_PAGE,
...
const handleDeleteRow = async (date, index) => {
  ...
  // 🔁 If it's a saved row with an _id, delete from backend
  if (rowToDelete._id) {
    try {
      await axiosInstance.delete(`/production-reports-block/delete/${rowToDelete._id}`, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
      toast.success("Row deleted successfully!");
    } catch (err) {
      console.error("Error deleting block row:", err);
      toast.error("Failed to delete row.");
      return;
    }
  }

  // 🧹 Remove the row from local groupedData
  updatedGrouped[date].splice(index, 1);

  // 🧾 Remove the date group if it's empty, else reindex srNo
  ...
  const handleSave = async () => {
    ...
    try {
      await axiosInstance.post(
        "/production-reports-block/block-moulding-update",
        flatData,
        {
          headers: { Authorization: `Bearer ${user.token}` },
        }
      );
      // ✅ Remove isNew from each row after successful save
      const cleanedGrouped = {};
      for (const date in groupedData) {
        cleanedGrouped[date] = groupedData[date].map(
          ({ isNew, ...rest }) => rest
        );
      }
  ...
  const paginatedDates = Object.keys(groupedData).sort(
                ...
                <thead className="bg-blue-600">
                  <tr>
                    {[
                      "Sr No",
                      "Date",
                      "Block Mould Density/Specification",
                      "Weight of Block(in kgs)",
                      "No of Blocks",
                      "Total Weight(in kgs)",
                      "Actions",
                    ].map((head) => (
                    ...
                    (() => {
                      ...
                      return paginatedDates.map((date) => {
                        ...
                        const totalBlocks = rows.reduce(
                          (sum, r) => sum + (parseInt(r.noOfBlocks) || 0),
                          0
                        );
                              ...
                              >
                                ...
                                <td className="px-2 py-1 text-center text-gray-700 bg-gray-100">
                                  {row.weightOfBlock && row.noOfBlocks
                                    ? (
                                        parseFloat(row.weightOfBlock) *
                                        parseInt(row.noOfBlocks)
                                      ).toFixed(2)
                                    : ""}{" "}
                                  kgs
                                </td>
                                 <td className="px-2 py-1 text-center">
  {(!row.mouldName && !row.weightOfBlock && !row.noOfBlocks) && (
    <button
