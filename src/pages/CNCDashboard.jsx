...
// ✅ Helper: Check if CNC slip has products
const hasCncSlipProducts = (order) => {
  return order.cncSlip?.products && order.cncSlip.products.length > 0;
};

// ✅ Helper: Get product details from CNC slip if available
const getProductDetailsFromSlip = (order) => {
  // First check if CNC slip has products
  if (hasCncSlipProducts(order)) {
    return order.cncSlip.products.map(p => ({
      name: p.productName,
      quantity: p.quantity,
      size: p.size || "N/A",
      remarks: p.remarks || "",
...
// ✅ Helper: Get total quantity from CNC slip if available
const getTotalQuantityFromSlip = (order) => {
  if (hasCncSlipProducts(order)) {
    return order.cncSlip.products.reduce((sum, p) => sum + (parseInt(p.quantity) || 0), 0);
  }
  ...
  {orders.map((order) => {
    ...
    return (
      <tr key={order._id} className="hover:bg-gray-50 transition duration-150">
        ...
        {/* Product Name - From CNC slip if available */}
  ...
  {(() => {
    ...
    return (
      <div className="space-y-1">
        {productList.map((p, idx) => (
          <div key={idx} className="border-b border-gray-200 pb-1 last:border-0">
            ...
            <div className="text-xs text-gray-500 mt-0.5">
              Qty: {p.quantity} | Size: {p.size || "N/A"}
              {p.drawingName && <span className="ml-2 text-purple-600">| Drawing: {p.drawingName}</span>}
       ...
       {/* Size - From CNC slip if available */}
<td className="px-4 py-2">
  {(() => {
    const productList = getProductDetailsFromSlip(order);
  ...
  {/* Quantity - From CNC slip if available */}
<td className="px-4 py-2">
  {(() => {
    const productList = getProductDetailsFromSlip(order);
    const isMulti = productList.length > 1;
    ...
    return (
      <span className="bg-blue-100 px-2 py-0.5 rounded">
       ...
       {/* Remarks - From CNC slip if available */}
<td className="px-4 py-2">
  {(() => {
    const productList = getProductDetailsFromSlip(order);
    const slipRemarks = order.cncSlip?.remarks || order.cncSlip?.slipRemarks;
