...
export default function WaterPollutionCertificate() {
  ...
  const fetchCertificates = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get(`/water-pollution-certificate?page=${page}&limit=10`);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const res = await axiosInstance.post("/water-pollution-certificate/upload", uploadData, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let res;
      if (editingId) {
        res = await axiosInstance.put(`/water-pollution-certificate/${editingId}`, submitData);
      } else {
        res = await axiosInstance.post("/water-pollution-certificate", submitData);
      }
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axiosInstance.delete(`/water-pollution-certificate/${id}`);
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">
              Water Pollution Certificate Management
            </h1>
