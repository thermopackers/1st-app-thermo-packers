...
export default function HardnessMonitoring() {
  ...
  const fetchLogs = async () => {
    try {
      ...
      const res = await axiosInstance.get(`/hardness?${params.toString()}`);
      setLogs(res.data.logs);
      setTotalPages(res.data.totalPages);
      setTotalRecords(res.data.totalRecords);
    } catch (err) {
      console.error("Error fetching Hardness logs:", err);
      Swal.fire("Error", "Failed to load Hardness data", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFileUpload = async (e, field, rowId) => {
    ...
    try {
      ...
      const uploadRes = await axiosInstance.post("/hardness/upload", uploadData, {
  ...
  const handleFileDelete = async (fileUrl, field, rowId, fileIndex) => {
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        const deleteRes = await axiosInstance.delete("/hardness/file", { 
          data: { fileUrl } 
        });
        ...
        if (deleteRes.data.success) {
          ...
          if (rowId !== 'new') {
            ...
            if (logToUpdate) {
              await axiosInstance.put(`/hardness/${rowId}`, logToUpdate);
            }
          }
    ...
    try {
      ...
      if (rowId === 'new') {
        ...
        const formattedData = {
          ...
          boreWaterHardness: {
            value: logData.boreWaterHardness?.value || "",
            files: logData.boreWaterHardness?.files || []
          },
          feedWaterHardness: {
            value: logData.feedWaterHardness?.value || "",
            files: logData.feedWaterHardness?.files || []
          },
          drainWaterHardness: {
            value: logData.drainWaterHardness?.value || "",
            files: logData.drainWaterHardness?.files || []
          },
          remarks: logData.remarks || ""
        };
        
        const res = await axiosInstance.post("/hardness", formattedData);
        
        fetchLogs();
        
        Swal.fire({
          title: "Success!",
          text: res.data.message || "New Hardness entry added.",
          icon: "success",
          timer: 2000
        });
        ...
        const formattedData = {
          ...
          date: logToSave.date,
          boreWaterHardness: {
            value: logToSave.boreWaterHardness?.value || "",
            files: logToSave.boreWaterHardness?.files || []
          },
          feedWaterHardness: {
            value: logToSave.feedWaterHardness?.value || "",
            files: logToSave.feedWaterHardness?.files || []
          },
          drainWaterHardness: {
            value: logToSave.drainWaterHardness?.value || "",
            files: logToSave.drainWaterHardness?.files || []
          },
          remarks: logToSave.remarks || ""
        };
        
        const res = await axiosInstance.put(`/hardness/${rowId}`, formattedData);
        
        Swal.fire({
          title: "Updated!",
          text: res.data.message || "Hardness entry updated.",
          icon: "success",
          timer: 2000
        });
      }
    } catch (err) {
      console.error("Error saving Hardness entry:", err);
  ...
  const handleAddRow = () => {
    const newRow = {
      ...
      boreWaterHardness: { value: "", files: [] },
      feedWaterHardness: { value: "", files: [] },
      drainWaterHardness: { value: "", files: [] },
      remarks: ""
    };
  ...
  const handleDeleteRow = async (rowId) => {
    ...
    const confirm = await Swal.fire({
      title: "Delete Entry?",
      text: "This will remove the Hardness entry permanently.",
      icon: "warning",
      showCancelButton: true,
      confirmButtonText: "Yes, delete it",
    });
    ...
    if (confirm.isConfirmed) {
      try {
        await axiosInstance.delete(`/hardness/${rowId}`);
        fetchLogs();
        Swal.fire("Deleted!", "Hardness entry removed.", "success");
      } catch (err) {
        console.error("Error deleting Hardness entry:", err);
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        {loading && (
          <div className="flex justify-center items-center py-8">
            ...
            <span className="ml-3 text-lg">Loading Hardness data...</span>
          </div>
        )}
          ...
          <div className="bg-white shadow-lg rounded-lg overflow-x-auto mb-6">
            <table className="min-w-full border-collapse">
              <thead className="bg-slate-200">
                <tr>
                  ...
                  <th className="border p-3 text-left text-xs">Hardness of Submersible Bore Water from Ground (ppm CaCO₃)</th>
                  <th className="border p-3 text-left text-xs">Hardness of Feed Water to Boiler from Softner Plant (ppm CaCO₃)</th>
                  <th className="border p-3 text-left text-xs">Hardness of Blow Down/Drain Water (ppm CaCO₃)</th>
                  ...
                  >
                    ...
                    <td className="border p-2">
                      <div className="space-y-2">
                        <input
                          type="text"
                          value={log.boreWaterHardness?.value || ""}
                          onChange={(e) => handleFieldChange(log._id, 'boreWaterHardness.value', e.target.value)}
                          className="border rounded p-1 w-full text-xs"
                          placeholder="Enter ppm CaCO₃ value"
                          disabled={savingRow === log._id}
                        />
                        {renderFileThumbnails(log.boreWaterHardness?.files || [], 'boreWaterHardness', log._id)}
                        <input
                          type="file"
                          multiple
                          accept="image/*,.pdf"
                          onChange={(e) => handleFileUpload(e, 'boreWaterHardness', log._id)}
                    ...
                    <td className="border p-2">
                      <div className="space-y-2">
                        <input
                          type="text"
                          value={log.feedWaterHardness?.value || ""}
                          onChange={(e) => handleFieldChange(log._id, 'feedWaterHardness.value', e.target.value)}
                          className="border rounded p-1 w-full text-xs"
                          placeholder="Enter ppm CaCO₃ value"
                          disabled={savingRow === log._id}
                        />
                        {renderFileThumbnails(log.feedWaterHardness?.files || [], 'feedWaterHardness', log._id)}
                        <input
                          type="file"
                          multiple
                          accept="image/*,.pdf"
                          onChange={(e) => handleFileUpload(e, 'feedWaterHardness', log._id)}
                          className="text-xs w-full bg-yellow-200 p-1 rounded"
                          disabled={savingRow === log._id}
                        />
                      </div>
                    ...
                    {/* Drain Water Hardness */}
                    <td className="border p-2">
                      <div className="space-y-2">
                        <input
                          type="text"
                          value={log.drainWaterHardness?.value || ""}
                          onChange={(e) => handleFieldChange(log._id, 'drainWaterHardness.value', e.target.value)}
                          className="border rounded p-1 w-full text-xs"
                          placeholder="Enter ppm CaCO₃ value"
                          disabled={savingRow === log._id}
                        />
                        {renderFileThumbnails(log.drainWaterHardness?.files || [], 'drainWaterHardness', log._id)}
                        <input
                          type="file"
                          multiple
                          accept="image/*,.pdf"
                          onChange={(e) => handleFileUpload(e, 'drainWaterHardness', log._id)}
                          className="text-xs w-full bg-yellow-200 p-1 rounded"
                          disabled={savingRow === log._id}
                        />
                      </div>
            ...
            {logs.length === 0 && !loading && (
              <div className="text-center py-8 text-gray-500 bg-white rounded-lg shadow">
                <p className="text-lg">No Hardness entries found.</p>
