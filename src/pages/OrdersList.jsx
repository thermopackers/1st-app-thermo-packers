...
const getCustomerPhone = (name) => {
  ...
  const sectionsList = [
    {
      key: "preExpander",
      label: "EPS/Thermocol Block Molding Production Section",
    },
    { key: "danaBeads", label: "EPS/Thermocol Dana/Beads Production Section" },
    {
      key: "shapeMoulding",
      label: "EPS/Thermocol Shape Molding Production Section",
    },
    {
      key: "sheetCutting",
      label: "EPS/Thermocol Sheet Cutting & Dispatch Section",
    },
    {
      key: "shapePackaging",
      label: "EPS/Thermocol Shape Molding Packaging & Dispatch Section",
    },
...
useEffect(() => {
  const checkAndCompleteOrders = async () => {
    for (const order of orders) {
      // Check if all three statuses indicate completion
...
}, [orders, token]); // This will run whenever orders change
  ...
  const groupedOrders = useMemo(() => {
    ...
    const sortedPOs = Object.entries(grouped).sort(([, a], [, b]) => {
      const latestA = a.reduce(
        (max, curr) => (curr._id > max._id ? curr : max),
        a[0]
      );
      const latestB = b.reduce(
        (max, curr) => (curr._id > max._id ? curr : max),
        b[0]
      );
      ...
      <div className="modal-content bg-white p-6 rounded-lg shadow-lg max-w-3xl w-full my-10 max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input
            ...
            name="freightAmount"
            value={updatedOrder.freightAmount}
            onChange={handleChange}
          />
          <Input
            label="Density"
            name="density"
            value={updatedOrder.density}
            onChange={handleChange}
          />
          <Input
            label="Packaging Charge"
            name="packagingCharge"
            value={updatedOrder.packagingCharge}
            onChange={handleChange}
          />
          <Input
            label="Size"
          ...
          <Input
            label="Bill To"
            name="billTo"
            value={updatedOrder.billTo}
            onChange={handleChange}
          />
          <Input
            label="Ship To"
            name="shipTo"
            value={updatedOrder.shipTo}
            onChange={handleChange}
          />
          <Input
            label="Remarks"
            name="remarks"
            value={updatedOrder.remarks}
            onChange={handleChange}
          />
          <Input
            label="Date"
