...
useEffect(() => {
  ...
  setRequiredDate(po.requiredDate ? po.requiredDate.slice(0, 10) : ""); // to match input[date] format
        ...
        const entries = po.products.map((p) => {
          const masterProduct = allProducts.find((prod) => prod.name === p.productName);
...
console.log("item",item);
    ...
    if (!item.name || !item.qty || !item.price || !item.gstPercent) {
      toast.error(`Please fill all required fields for Product ${i + 1} (Remarks, Payment Comment, and Freight Comment are optional).`);
      return; // Exit the function if any required field is missing
    }

   if (!freightOption && !freightComment) {
  toast.error("Please select a Freight Option or enter a Freight Comment.");
  return;
}
...
let grandTotal = 0;
  ...
  for (let i = 0; i < productEntries.length; i++) {
    ...
    const imageUrls = product?.files || []; // Get the full files array
      ...
      const totalAmount = parseFloat(item.qty || 0) * parseFloat(item.price || 0);
...
,
  ...
  columns: [
    ...
    { width: "*", stack: [{ text: `Payment Term: ${paymentTerm || paymentComment || "-"}`, fontSize: 8 , bold: true, }] },
    { width: "*", stack: [{ text: `Required Date: ${requiredDate || "-"}`, fontSize: 8 , bold: true, }] },
  ],
  margin: [0, 0, 0, 10],
},
                  {
...
widths: [20, "*", 40, 40, 40, 50, 40, 50, "*"], // 120 for description column
                  ...
                  { text: "TERMS AND CONDITIONS", style: "termsTitle", margin: [0, 0, 0, 10], fontSize: 9 },
                  { ul: termsAndConditions, fontSize: 7 },
                {
  stack: [
    { text: "\nFor THERMO PACKERS", alignment: "right", fontSize: 8 },
    { text: `Generated by: ${user?.name || "-"}`, alignment: "right", fontSize: 7, italics: true },
  ],
  margin: [0, 30, 0, 0],
},
...
const logoUrl = "https://res.cloudinary.com/dcr8k5amk/image/upload/v1753784825/THERMO_PACKERS_12032021-01_q181qx.jpg";
...
if (id && existingPdfUrl) {
  ...
  const resourceType = existingPdfUrl.includes("/raw/") ? "raw" : "image";

  try {
    await axiosInstance.post("/cloudinary/delete-file", { publicId, resourceType });
...
await axiosInstance.post("/purchase-orders/save", {
 ...
 navigate("/purchase-orders");
  ...
  return (
    <>
      <LoaderOverlay isLoading={isUploading} /> {/* Add the overlay component here */}
      <InternalNavbar />
...
>
  ...
  <div>
    ...
    >
      ...
      <option value="Picked by Thermo Packers">Picked by Thermo Packers</option>
    </select>

    <label className="block font-medium mt-2 mb-1">Freight Comment (Optional)</label>
    <input
      type="text"
      className="border p-2 rounded w-full"
  ...
  <div>
    ...
    <label className="block font-medium mt-2 mb-1">Payment Comment (Optional)</label>
    <input
      type="text"
      className="border p-2 rounded w-full"
  ...
  <div>
    <label className="block font-medium mb-1">Material Required Date</label>
    <input
      type="date"
      className="border p-2 rounded w-full"
      value={requiredDate}
    ...
    />
        ...
        {productEntries.map((item, index) => (
          <div key={index} className="mb-6 border border-gray-300 rounded p-4 shadow-sm bg-white relative">
            {/* ❌ Delete Button */}
            ...
            <h3 className="font-semibold mb-3 text-lg text-blue-700">Product {index + 1}</h3>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {/* Product Name */}
              <div>
                <label htmlFor={`product-${index}`} className="block font-medium mb-1">Product Name</label>
                <select
                  id={`product-${index}`}
                  className="border p-2 rounded w-full"
                  value={item.name}
...
<div>
  ...
  />
                ...
                {/* Qty */}
      <div>
        <label htmlFor={`qty-${index}`} className="block font-medium mb-1">
          Quantity (kg)
        </label>
        <input
          id={`qty-${index}`}
...
{/* Unit */}
<div>
  <label htmlFor={`unit-${index}`} className="block font-medium mb-1">
    Unit (e.g., Kg, Nos)
  </label>
  <input
    id={`unit-${index}`}
  ...
  />
      ...
      {/* GST */}
      <div>
        <label htmlFor={`gst-${index}`} className="block font-medium mb-1">
          GST (%)
        </label>
        <input
          id={`gst-${index}`}
      ...
      {/* Remarks */}
      <div>
        <label htmlFor={`remarks-${index}`} className="block font-medium mb-1">
          Remarks
        </label>
        <input
          id={`remarks-${index}`}
