...
export default function AirReceiverCertificate() {
  ...
  const fetchCertificates = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get(`/air-receiver-certificate?page=${page}&limit=10`);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const res = await axiosInstance.post("/air-receiver-certificate/upload", uploadData, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let res;
      if (editingId) {
        res = await axiosInstance.put(`/air-receiver-certificate/${editingId}`, submitData);
      } else {
        res = await axiosInstance.post("/air-receiver-certificate", submitData);
      }
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axiosInstance.delete(`/air-receiver-certificate/${id}`);
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">
              Air Receiver Tank Certificate Management
            </h1>
