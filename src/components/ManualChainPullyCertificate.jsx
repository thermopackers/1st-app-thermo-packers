...
export default function ManualChainPullyCertificate() {
  ...
  const fetchCertificates = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get(`/manual-chain-pully-certificate?page=${page}&limit=10`);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const res = await axiosInstance.post("/manual-chain-pully-certificate/upload", uploadData, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let res;
      if (editingId) {
        res = await axiosInstance.put(`/manual-chain-pully-certificate/${editingId}`, submitData);
      } else {
        res = await axiosInstance.post("/manual-chain-pully-certificate", submitData);
      }
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axiosInstance.delete(`/manual-chain-pully-certificate/${id}`);
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">
              Manual Chain Pully Block Certificate Management
            </h1>
