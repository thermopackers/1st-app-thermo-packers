...
const FloatingWhatsApp = () => {
  ...
  const captureLead = async () => {
    ...
    try {
      // Get user data from session storage or use default values
      const userData = {
        name: sessionStorage.getItem('user_name') || 'WhatsApp User',
        email: sessionStorage.getItem('user_email') || 'user@example.com',
      ...
      // Save to database
      const response = await axiosInstance.post('/leads/whatsapp', userData);
      ...
      // Continue to open WhatsApp even if save fails
    } finally {
      setLoading(false);
    }
    return false;
  ...
  const handleWhatsAppClick = async () => {
    // Capture lead data if not already submitted
    if (!leadSubmitted) {
      await captureLead();
    }
